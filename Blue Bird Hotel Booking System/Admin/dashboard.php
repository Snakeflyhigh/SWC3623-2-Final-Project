...
<?php 	
    ...
    {
        $profit = $row["finaltotal"] * 10 / 100; // 10% profit
        $profit_myr = number_format($profit, 2, '.', ''); // Format as MYR (2 decimal places)
        
        $chart_data .= "{ date:'".$row["cout"]."', profit:".$profit_myr ."}, ";
        $tot = $tot + $profit;
    }
...
<body>
   <div class="databox">
        ...
        <h2>Profit</h1>  
          <h1>RM<?php echo number_format($tot, 2, '.', ','); ?></h1>
        </div>
    </div>
    <div class="chartbox">
